...
public:
   ...
   PlayCDDAData(const std::vector<TrackInfo> ti, bool gap) : /*stream(NULL), */
      frameOffset(0), theCD(NULL), trackList(ti), playing(false),
      endOfTrack(false), pregap(gap)
   {
      memset(nullAudio, 0, sizeof(nullAudio));
      volume = 1; //atof(prefs.prefsMap[volumeString].c_str());
      if (volume < 0) volume = 0;
      else if (volume > 1) volume = 1;
   }
		// cleans up
   virtual ~PlayCDDAData() {if (playing) stop(); delete theCD;}
   ...
   bool endOfTrack;
      // true if there's a missing 2 second pregap
   bool pregap;
};
